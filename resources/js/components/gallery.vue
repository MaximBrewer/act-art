        ...
        <div class="row">
          <div class="col-xs-6">
            <div class="g-title">
              ...
              <span class="ia letter">В</span>
              <h3 class="h3">Виртуальная галерея</h3>
              <p class="description">Более 50 000 работ от настоящих мастеров своего дела</p>
              <span class="arti arti-quotes"></span>
            </div>
          </div>
        </div>
      </div>
      <div class="gallery-inner" :style="{transform: transformGalleryInner}" ref="galleryInner">
        <block-it
          v-for="(block, j) in itemsBlocks"
          :key="j"
          :block="block"
        ></block-it>
      </div>
...
export default {
  ...
  data: function() {
    return {
      itemsBlocks: [],
      transformGalleryInner: "translate3d(0,0,0)",
      stopFlag: 0,
      visibleFlag: 0,
      loading: !1,
      loadingPercent: 0.3,
  ...
  mounted: function() {
    let app = this;
    app.galInner = this.$refs.galleryInner;
    axios
      .get(
      ...
      .then(response => {
        ...
        app.stopFlag = parseInt(response.stopFlag);
        app.itemsBlocks.push(response.data);
        let o = 0;
        for (let j in app.$refs.galleryBody) {
          o += app.$refs.galleryBody[j].offsetWidth;
        }
        app.galInner.style.width = o;
        app.width = o;
        app.loading = !1;
      });
  },
  methods: {
    animate: function() {
      ...
      app.animateGallery({
        loadPhoto: function() {
          if (((app.loading = !0), !app.stopFlag)) {
            axios
              ...
              .then(response => {
                app.stopFlag = parseInt(response.stopFlag);
                app.itemsBlocks.push({ items: response.gallery });
                let o = 0;
                for (let j in app.$refs.galleryBody) {
                  o += app.$refs.galleryBody[j].offsetWidth;
                }
        ...
        moveLeft: function(t, e) {
          var i = t - e;
          app.transform = "translate3d(" + i + "px, 0px, 0px)";
        }
      });
    },
    initGrids(t, e) {
      ...
      return (
        n.width(a),
        n.parent().width(a),
        n.BlocksIt({ offsetX: 0, offsetY: 0, numOfCol: r }).queue(function() {
          s.each(function() {
    ...
    getTranslateX() {
      if ("" === this.transformGalleryInner) return 0;
      var i = this.transformGalleryInner
  ...
  //         (Gallery.initGrids($(".gallery-body:first-child"), 240),
  //         new Waypoint.Inview({
